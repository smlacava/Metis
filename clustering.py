...
class clustering():
    def __init__(self, clusters=2):
        ...
        :param clusters: it is the number of clusters (2 by default)
        """
    ...
    def set_clusters(self, clusters):
        ...
        :param clusters: it is the number of clusters
        """
    ...
    def compute_clusters(self, data, clusters=None):
        ...
        :param clusters: it is the number of cluster to evaluate, or None to use the previously inserted clusters (None
                         by default
    ...
    def clustering_plot(self, data, y_pred=None, view=True, save=False):
        """
        ...
        :param data:   it is the 2D (samples*features) data matrix
        :param y_pred: it is the array containing the cluster for each subject, or None to consider each as belonging to
                       the same subject (None by default)
        :param view:   it has to be True in order to show the resulting plots, False otherwise (True by default)
        :param save:   it has to be True in order to save the resulting figure as clustering.png, False othersise (False
                       by default
        """
        ...
        for f1 in range(features):
            for f2 in range(features):
                ...
                if f2 == 0:
                    m = np.min(data[:, f2])
                    plt.yticks([m+(np.max(data[:, f2])-m)/2], [str(f1)])
    ...
    def cluster_analysis(self, data, clusters=None, view=True, save=False):
        """
        ...
        self.clustering_plot(data, y_pred, view, save)
    ...
    def cluster_performance(self, data, labels):
        ...
        :param data:     it is the 2D (samples*features) data matrix
        :param labels:   it is the list of real labels.
        :return:         the accuracy
        """
